...
Yolo11ObjDet::Yolo11ObjDet(const std::string& engine_file_path) {
...
void Yolo11ObjDet::make_pipe(bool warmup) {
...
// 将图像调整到深度学习需要的尺寸
void Yolo11ObjDet::letterbox(const cv::Mat& image, cv::Mat& out, cv::Size& size) {
...
void Yolo11ObjDet::copy_from_Mat(const cv::Mat& image) {
    cv::Mat nchw;
...
void Yolo11ObjDet::copy_from_Mat(const cv::Mat& image, cv::Size& size) {
    cv::Mat nchw;
    this->letterbox(image, nchw, size);
...
void Yolo11ObjDet::postprocess(std::vector<Object>& objs, float score_thres, float iou_thres, int topk, int num_labels) {
    objs.clear();
...
void Yolo11ObjDet::draw_objects(const cv::Mat& image, cv::Mat& res, const std::vector<Object>& objs,
