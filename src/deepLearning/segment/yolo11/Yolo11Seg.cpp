...
Yolo11Seg::Yolo11Seg(const std::string &engine_file_path) {
    ...
    for (int i = 0; i < this->num_bindings; ++i) {
        BindingYolo11Seg binding;
        nvinfer1::Dims dims;
        nvinfer1::DataType dtype = this->engine->getBindingDataType(i);
        std::string name = this->engine->getBindingName(i);
        binding.name = name;
        binding.dsize = typeToSizeYolo11Seg(dtype);

        bool IsInput = engine->bindingIsInput(i);
        if (IsInput) {
            this->num_inputs += 1;
            dims = this->engine->getProfileDimensions(i, 0, nvinfer1::OptProfileSelector::kMAX);
            binding.size = getSizeByDimsYolo11Seg(dims);
            binding.dims = dims;
...
void Yolo11Seg::make_pipe(bool warmup) {
...
void Yolo11Seg::letterbox(const cv::Mat &image, cv::Mat &out, cv::Size &size) {
...
void Yolo11Seg::copy_from_Mat(const cv::Mat &image) {
    cv::Mat nchw;
...
void Yolo11Seg::copy_from_Mat(const cv::Mat &image, cv::Size &size) {
    cv::Mat nchw;
    this->letterbox(image, nchw, size);
    this->context->setBindingDimensions(0, nvinfer1::Dims{
...
void Yolo11Seg::infer() {
    this->context->enqueueV2(this->device_ptrs.data(), this->stream, nullptr);
    for (int i = 0; i < this->num_outputs; i++) {
        size_t osize = this->output_bindings[i].size * this->output_bindings[i].dsize;
        CHECK(
            cudaMemcpyAsync(this->host_ptrs[i], this->device_ptrs[i + this->num_inputs], osize, cudaMemcpyDeviceToHost, this->stream));
    }
    cudaStreamSynchronize(this->stream);
}

void Yolo11Seg::postprocess(std::vector<ObjectYolo11Seg> &objs, float score_thres, float iou_thres, int topk, int seg_channels,
                            int seg_h, int seg_w) {
    objs.clear();
    ...
    for (int i = 0; i < num_anchors; i++) {
        ...
        if (score > score_thres) {
            ...
            float x0 = clampYolo11Seg((x - 0.5f * w) * ratio, 0.f, width);
            float y0 = clampYolo11Seg((y - 0.5f * h) * ratio, 0.f, height);
            float x1 = clampYolo11Seg((x + 0.5f * w) * ratio, 0.f, width);
            float y1 = clampYolo11Seg((y + 0.5f * h) * ratio, 0.f, height);
    ...
    if (masks.empty()) {
        ...
        // 检查总元素数量是否匹配
        int totalElements = matmulRes.total();
        int requiredElements = indices.size() * seg_h * seg_w;

        if (totalElements != requiredElements) {
            std::cerr << "Error: Total elements (" << totalElements << ") do not match required elements (" << requiredElements << ")."
                      << std::endl;
            // 如果元素数量不匹配，返回空矩阵，避免崩溃
            maskMat = cv::Mat();
        } else {
            try {
                // 安全执行 reshape
                maskMat = matmulRes.reshape(indices.size(), {seg_h, seg_w});
            } catch (const cv::Exception &e) {
                std::cerr << "OpenCV Exception during reshape: " << e.what() << std::endl;
                // 发生异常时返回空矩阵
                maskMat = cv::Mat();
            }
        }
...
void Yolo11Seg::draw_objects(const cv::Mat &image, cv::Mat &res, const std::vector<ObjectYolo11Seg> &objs,
    ...
    cv::addWeighted(res, 0.5, mask, 0.5, 1, res);
}
