...
HandEyeCalibration::HandEyeCalibration(const std::string& imgsDirector, const std::string& outputFilename, cv::Size boardSize,
                                       double squareSize, Pattern type)
    : Calibration(imgsDirector, outputFilename, boardSize, squareSize, type) {}

HandEyeCalibration::~HandEyeCalibration() {}

void HandEyeCalibration::computerHandEyeMatrix(cv::Mat& Hcg, std::vector<cv::Mat>& Hgij, std::vector<cv::Mat>& Hcij,
...
void HandEyeCalibration::Tsai_HandEye(cv::Mat& Hcg, std::vector<cv::Mat>& Hgij, std::vector<cv::Mat>& Hcij) {
    ...
    int nStatus = static_cast<int>(Hgij.size());
    ...
    cv::Mat PcgTrs = Pcg.t();
    cv::Mat Rcg = (1 - norm(Pcg) * norm(Pcg) / 2) * eyeM + 0.5 * (Pcg * PcgTrs + sqrt(4 - norm(Pcg) * norm(Pcg)) * skew(Pcg));
...
void HandEyeCalibration::convertVectors2Hij(std::vector<cv::Mat>& vecHg, std::vector<cv::Mat>& vecHc, std::vector<cv::Mat>& vecHgij,
...
bool HandEyeCalibration::readDatasFromFile(const std::string& file, std::vector<cv::Mat>& vecHg, std::vector<cv::Mat>& vecHc) {
    ...
    while (!infile.eof()) {
        ...
        for (int i = 0; i < 6; ++i) infile >> temp.at<double>(0, i);
        // cout << temp << endl;
    ...
    if (sz % 2 == 1) {
        std::cout << " data items number not match." << std::endl;
        return false;
    }
    ...
    /////////////////////////////////////////
    // vector to matrix
    for (auto& m : vecHg) m = attitudeVectorToMatrix(m, "zyx");  // euler angles -> r matrix

    for (auto& m : vecHc) m = attitudeVectorToMatrix(m);  // r vec -> r matrix

    return true;
}

/** @brief 从配置文件中读取相机到物体的Rt以及基座到机械手末端的Rt
 *
 *	@note
 *
 *	@param 	file				存放相机参数的配置文件
 *	@param 	folder				存放机械臂末端坐标值信息的配置文件 xml文件
 *	@param 	vecHg				n组末端姿态到基座标的Rt矩阵 4*4
 *	@param 	vecHc				n组棋盘到相机坐标系的Rt矩阵   4*4
 *	@param 	useQuaternion		机械臂末端坐标值信息中是否通过四元数存储的
 *
 */
bool HandEyeCalibration::readDatasFromFile(const std::string& file, const std::string& folder, std::vector<cv::Mat>& vecHg,
                                           std::vector<cv::Mat>& vecHc, bool useQuaternion) {
    ...
    // 文件名必须是"robotpos##.xml"，这是lamuda表达式
    auto getSubstr = [](const cv::String& str) {   // nums "##" in "robotpos##.xml"
        size_t strPos = str.find("robotpos") + 8;  // 8位字符串
        size_t dotPos = str.find_last_of(".");
        return str.substr(strPos, dotPos - strPos);
    };
    std::sort(xmlFiles.begin(), xmlFiles.end(),
              [&getSubstr](cv::String& lhs, cv::String& rhs) { return std::stoi(getSubstr(lhs)) < std::stoi(getSubstr(rhs)); });

    // check items
    if (xmlFiles.size() == 0) {
        std::cout << folder << "has no xml files." << std::endl;
        return false;
    } else if (xmlFiles.size() < (int)foundCheeseBoardMat.total()) {
        std::cout << folder << "xml files items do not match found cheese'es count." << std::endl;
        return false;
    }
    ...
    for (int i = 0; i < extBigMat.rows; ++i) {
        cv::Mat tmp = attitudeVectorToMatrix(extBigMat.row(i), false);  // native cv Rodrigues  旋转向量转旋转矩阵
        vecHc.push_back(std::move(tmp));                                // extrinsic matrix
    }
    for (int i = 0; i < attitudeBigMat.rows; ++i) {
        // 共三组标定实例，其中第一组给的pose中的旋转部分为旋转向量， 第二和第三对应的是欧拉角
        // cv::Mat tmp = attitudeVectorToMatrix(attitudeBigMat.row(i), useQuaternion, "");  // 旋转向量  对应1
        cv::Mat tmp = attitudeVectorToMatrix(attitudeBigMat.row(i), useQuaternion, "xyz");  // 欧拉角 对应2和3    常用
 ...
 *
 */
bool HandEyeCalibration::readRobotDatasFromFile(const std::string& folder, std::vector<cv::Mat>& vecHg, bool useQuaternion) {
    // 位姿大矩阵
    cv::Mat attitudeBigMat;
    std::vector<cv::String> xmlFiles;
    cv::glob(folder + "/*.xml", xmlFiles);  // specific prefix "robotpos##.xml"
    ...
    for (int i = 0; i < xmlFiles.size(); ++i) {
        cv::FileStorage fs(xmlFiles[i], cv::FileStorage::READ);
        if (!fs.isOpened()) {
            std::cout << "Can not open \"" << xmlFiles[i] << "\"." << std::endl;
            return false;
        }
        std::cout << "Robot_pos: " << xmlFiles[i] << std::endl;
        std::vector<double> tmp_6;  // specific items 6
        for (int i = 0; i < 6; ++i) {
            double val = fs[cv::format("Position%d", i)].real();
            tmp_6.push_back(val);
    ...
    for (int i = 0; i < attitudeBigMat.rows; ++i) {
        // 共三组标定实例，其中第一组给的pose中的旋转部分为旋转向量， 第二和第三对应的是欧拉角
        // cv::Mat tmp = attitudeVectorToMatrix(attitudeBigMat.row(i), useQuaternion, "");  // 旋转向量  对应1
        cv::Mat tmp = attitudeVectorToMatrix(attitudeBigMat.row(i), useQuaternion, "xyz");  // 欧拉角 对应2和3   常用"xyz"

        tmp = tmp.inv();  // 根据眼在手外公式推导，此处需取逆转换矩阵关系
...
cv::Mat HandEyeCalibration::attitudeVectorToMatrix(cv::Mat m, bool useQuaternion, const std::string& seq) {
...
cv::Mat HandEyeCalibration::eulerAngleToRotatedMatrix(const cv::Mat& eulerAngle, const std::string& seq) {
    ...
    if (seq == "zyx")
        ...
        rotMat = rotZ * rotY * rotZ;  // 新添加
    else {
        cv::error(cv::Error::StsAssert, "Euler angle sequence string is wrong.", __FUNCTION__, __FILE__, __LINE__);
    }

    if (!isRotationMatrix(rotMat)) {
        cv::error(cv::Error::StsAssert, "Euler angle can not convert to rotated matrix", __FUNCTION__, __FILE__, __LINE__);
    }
...
cv::Mat HandEyeCalibration::quaternionToRotatedMatrix(const cv::Vec4d& q) {
    ...
    cv::Matx33d res{
        1 - 2 * (y2 + z2), 2 * (xy - wz), 2 * (xz + wy), 2 * (xy + wz),     1 - 2 * (x2 + z2),
        2 * (yz - wx),     2 * (xz - wy), 2 * (yz + wx), 1 - 2 * (x2 + y2),
    };
 ...
 *	@return 空间三维点坐标
 */
cv::Point3d HandEyeCalibration::getWorldPos(cv::Point2d& imgPt, double z, cv::Mat& Hcg, cv::Mat& Hg, cv::Mat& camMatrix) {
...
/*从配置文件中获取相机内参和畸变矩阵*/
bool HandEyeCalibration::readCameraParameters(const std::string& filename, cv::Mat& camMatrix, cv::Mat& distCoefs) {
...
/** @brief 求解eye-in-hand的相机坐标系与手臂坐标系(末端姿态)的Rt
 *
 *	@note
 *
 *	@param 	Hcg					待求值,手眼标定变换矩阵 4*4
 *	@param 	vecHg				n组末端姿态到基坐标系的Rt矩阵 4*4  阵
 *	@param 	vecHc				n组棋盘到相机坐标系的Rt
 *	@param 	method				使用的算法     HAND_EYE_NAVY：0   HAND_EYE_TSAI：1
 *	@param 	file				Hcg保存到的文件名
 *
 */
void HandEyeCalibration::calibrateEyeInHand(cv::Mat& Hcg, std::vector<cv::Mat>& vecHg, std::vector<cv::Mat>& vecHc,
                                            HandEyeCaliMethod method, const std::string& file) {
    // 求An,Bn
...
// https://github.com/wanggao1990/HandEyeCalibration/blob/fa3d161b554702b0e670291a6e842c102efb58b0/calibration/HandEyeCalibration.cpp
