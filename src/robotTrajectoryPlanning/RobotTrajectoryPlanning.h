...
public:
     ...
     * 定义『焊接时』机械臂『后几个轴相对于基座』朝向的方向是前方, 并以此时的左右作为左方和右方,
     * 『与机器人初始位置的前后左右无关』
     */
    ...
    std::array<double, 3> rotateVector(const std::array<std::array<double, 3>, 3>& mat,
                                       const std::array<double, 3>& vec);  // 旋转向量
    std::array<double, 3> abcToVector(double A, double B, double C);       // 将A、B、C应用到向量 (0, 0, 1)

    void write2File(const std::vector<std::shared_ptr<WeldSeamInfo>>& weldSeamInfo, int endOfLeftSeams) override;  // 焊缝写入文件
