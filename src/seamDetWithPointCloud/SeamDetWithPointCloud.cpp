...
// #include "seamDetWithPointCloud/beamButtSeamsDet/BeamButtSeamsDet.h"
// #include "seamDetWithPointCloud/cornerButtSeamsDet/CornerButtSeamsDet.h"
// #include "seamDetWithPointCloud/downBeamFilletSeamsDet/DownBeamFilletSeamsDet.h"
// #include "seamDetWithPointCloud/upBeamFilletSeamsDet/UpBeamFilletSeamsDet.h"
...
void SeamDetWithPointCloud::whenDetSeamWithPointCloud(std::vector<std::shared_ptr<WeldSeamInfo>> weldAreaInfo) {
    ...
    //     PLOGD << "区域编号: " << info->areaNum << ", 焊缝类型: " << info->weldType << ", 检测是否成功: " <<
    //     info->detectSuccFlag; if (info->detectSuccFlag == true) {
    //         PLOGD << "   焊缝端点坐标: (" << info->weldEndPointsInCamera->at(0).x << " " <<
    //         info->weldEndPointsInCamera->at(0).y
